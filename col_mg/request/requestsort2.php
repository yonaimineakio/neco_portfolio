...
//申請日の新しい、古い順にデーターをnarabikae

 if($new_or_old_order=="2"){

  $order_select='<div>新旧で並び替え<select name="request_date" onchange="submit(this.form)"><option value="1" >新しい</option><option value="2" selected>古い</option></select></div>';

 }else{

  $order_select='<div>新旧で並び替え<select name="request_date" onchange="submit(this.form)"><option value="1" selected>新しい</option><option value="2">古い</option></select></div>';
  }
